(function (global) {
	...
	Time.format = function (time, format, culture) {
		...
		format = replaceIfNotInBraces(format, "fff", function () {
				return padLeft(3, time.getMilliseconds());
			});
		format = replaceIfNotInBraces(format, "ff", function () {
				return padLeft(2, time.getMilliseconds());
			});
		format = replaceIfNotInBraces(format, "f", function () {
	...
	Date.isValid = function isValid(dateTxt, format) {
		return parseDate(dateTxt, format) != null;
	};
	Date.now = function(){
		return new Date();
	}
	...
	function getMatch(text, format, value) {
		var index = format.indexOf(value);
		if (index <= -1)
			return null;
		return text.substr(index, value.length);
	}
